...
from jpegdna.format import QuantizationTablesInfoFormatter
...
def generalinfoformatter_test():
    ...
    image_type = "gray"
    sampler = None
    formatter = GeneralInfoFormatter(freq_origin,
                                     m,
                                     n,
                                     blockdims,
                                     max_cat,
...
def quantizationtablesformatter_test():
    """Functionnal tests for the quantization tables formatter"""
    header = "ATCGATC"
    inp = np.zeros((8, 8))
    formatter = QuantizationTablesInfoFormatter((8, 8), 'gray', header)
    oligos = formatter.format(inp)
    oligos_in = []
    for oligo in oligos:
        oligos_in.append(oligo[7:])
    out = formatter.deformat(oligos_in)
    assert (inp == out).all()
    inp = (np.zeros((8, 8)), np.zeros((8, 8)), np.zeros((8, 8)))
    formatter = QuantizationTablesInfoFormatter((8, 8), 'RGB', header)
    oligos = formatter.format(inp)
    oligos_in = []
    for oligo in oligos:
        oligos_in.append(oligo[7:])
    out = formatter.deformat(oligos_in)
    for i in range(3):
        assert (inp[i] == out[i]).all()
    inp = (np.zeros((8, 8)), np.zeros((8, 8)), np.zeros((8, 8)))
    formatter = QuantizationTablesInfoFormatter((8, 8), 'RGB', header, debug=True)
    oligos = formatter.format(inp)
...
def jpegdna_test():
    ...
    debug = False
    choice = "from_img"
    formatter = JpegDNAFormatter(np.zeros((8,8)), "gray", primer="illumina", oligo_length=200, debug=debug)
    oligos = formatter.full_format(image_strand, choice, 512, 512, np.zeros((11)).astype(int), np.zeros((162)).astype(int))
    data_strand, (gammas, m, n, freq_dc, freq_ac) = formatter.full_deformat(oligos)
    assert data_strand[:len(image_strand)] == image_strand
    assert (gammas == np.zeros((8,8))).all()
    assert m == 512
    assert n == 512
...
def jpegdna_failure_value_primer_test():
    ...
    formatter = JpegDNAFormatter(np.zeros((8,8)), "gray", primer="coucou", oligo_length=200, debug=False)
...
def jpegdna_failure_value_freq_origin_test():
    ...
    _ = JpegDNAFormatter(np.zeros((8,8)), "gray", primer="coucou", oligo_length=200, debug=False)
@expected_value_error
def jpegdna_failure_value_set_freq_origin_test():
    """Failure tests for the general gray jpegdna formatter: set_freq_origin"""
    choice = "default"
    image_strand = "ATCG" * 10000
    formatter = JpegDNAFormatter(np.zeros((8,8)), "gray", primer="illumina", oligo_length=200, debug=False)
    _ = formatter.full_format(image_strand, choice, 512, 512, np.zeros((11)).astype(int), np.zeros((162)).astype(int))
...
def jpegdnargb_test():
    ...
    debug = False
    choice = "from_img"
    quant = (np.zeros((8,8)), np.zeros((8,8)), np.zeros((8,8)))
    formatter = JpegDNAFormatter(quant, "RGB", primer="illumina", oligo_length=200, debug=debug)
